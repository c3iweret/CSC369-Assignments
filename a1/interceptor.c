...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	//if syscall monitors some processes, check pid list and log message
	...
	//if syscall monitors all processes, log message
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	// checks the validity of 'syscall'
	if (syscall < 0 || syscall > NR_syscalls || syscall == MY_CUSTOM_SYSCALL) {
		return -EINVAL;
	}

	// -----------------------------------------------------------
	// Does all error checking, data tracking and synchronization 
	// for REQUEST_SYSCALL_INTERCEPT and REQUEST_SYSCALL_RELEASE
	// -----------------------------------------------------------
	if (cmd == REQUEST_SYSCALL_INTERCEPT || cmd == REQUEST_SYSCALL_RELEASE) {
		...
		if (cmd == REQUEST_SYSCALL_INTERCEPT) {
			...
			// error checking for REQUEST_SYSCALL_INTERCEPT
			if (table[syscall].intercepted == 1) {
				spin_unlock(&calltable_lock);
				return -EBUSY;

			} else {
				// Replaces original system call in the kernel
				// system call table with interceptor function
				// (at index 'syscall') 
			...
			// error checking for REQUEST_SYSCALL_RELEASE
			if (table[syscall].intercepted == 0) {
				...
				// Restores original system call in the kernel
				// system call table (at index 'syscall')
	...
	// -----------------------------------------------------------
	// Does all error checking, data tracking and synchronization 
	// for REQUEST_START_MONITORING and REQUEST_STOP_MONITORING
	// -----------------------------------------------------------
	if (cmd == REQUEST_START_MONITORING || cmd == REQUEST_STOP_MONITORING) {
		...
		if (current_uid() != 0) {
			...
			//permission denied for nonroot monitor all processes
			if (pid == 0 ) {
				return -EPERM;
			}
			
			//permission denied if not owning process
			else if((check_pid_from_list(current->pid, (pid_t) pid) == -EPERM)){
				return -EPERM;
			}
		}
		...
		if (cmd == REQUEST_START_MONITORING) {
			...
			//check is pid is already being monitored
			if (check_pid_monitored(syscall, (pid_t) pid) == 1) {
				...
				//update table and destroy pid list
				...
				int rem_pid_result = del_pid_sysc((pid_t) pid, syscall);
				spin_unlock(&pidlist_lock);
				return rem_pid_result;
			...
			//check that the process is being monitored
			else if ((check_pid_monitored(syscall, (pid_t) pid) == 0)) {
				spin_unlock(&pidlist_lock);
				return -EINVAL;
				...
				//Add pid to 'blacklist' of pids not monitored
				if(add_pid_sysc((pid_t)pid, syscall) != 0){
					spin_unlock(&pidlist_lock);
					return -ENOMEM;
				}
				...
				int deleted_pid_result = del_pid_sysc((pid_t) pid, syscall);
				spin_unlock(&pidlist_lock);
				return deleted_pid_result;
...
static int init_function(void) {
	...
	// stores original system calls
	orig_custom_syscall = sys_call_table[MY_CUSTOM_SYSCALL];
	orig_exit_group = sys_call_table[__NR_exit_group];

	// initializes and acquires spinlock
	spin_lock_init(&calltable_lock);
	spin_lock(&calltable_lock);

	// sets system call table to write mode
	set_addr_rw((unsigned long) sys_call_table);

	// hijacks the original system calls
	...
	// resets system call table to read only mode
	set_addr_ro((unsigned long) sys_call_table);
...
static void exit_function(void)
{
	...
	// restores the original system calls
