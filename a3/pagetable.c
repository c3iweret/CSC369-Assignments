...
int allocate_frame(pgtbl_entry_t *p) {
	...
	// attempts to find an unused memory block in memsize
	for(i = 0; i < memsize; i++) {
		if(!coremap[i].in_use) {
			frame = i;
			break;
	...
	if(frame == -1) { // Didn't find a free page.
		...
		if ((coremap[frame].pte)->frame & PG_DIRTY) {
			...
			// only a 'dirty' victim page is written to the swap file
			int evicted_file_offset = swap_pageout(
				((coremap[frame].pte)->frame >> PAGE_SHIFT), 
				(coremap[frame].pte)->swap_off);
			
			// checks if victim page is successfully written to swap file
			if (evicted_file_offset == INVALID_SWAP) {
				exit(1);
			}
			
			// clears out frame (address space) and does necessary bookkeeping 
			// for evicted page
			coremap[frame].pte->frame = 0;
			(coremap[frame].pte)->swap_off = evicted_file_offset;
			(coremap[frame].pte)->frame = (coremap[frame].pte)->frame | PG_ONSWAP;
			evict_dirty_count++;

		}
		
		// simply clears out frame for a 'clean' victim page 
		else {
			evict_clean_count++;
			coremap[frame].pte->frame = 0;
		}

		// the victim page is no longer in memory, or no longer 'valid'
...
char *find_physpage(addr_t vaddr, char type) {
	...
	// initializes the PDE if it is invalid (not initialized yet)
	if (!(pgdir[idx].pde & PG_VALID)) {
		pgdir[idx] = init_second_level();
	}

	// Use vaddr to get index into 2nd-level page table and initialize 'p'
	pgtbl_entry_t *page_tbl = (pgtbl_entry_t *) (pgdir[idx].pde & PAGE_MASK);
	p = &page_tbl[PGTBL_INDEX(vaddr)];
	...
	if (!(p->frame & PG_VALID)) {
		...
		//set addr of frame to virtual address(for OPT)
		coremap[frame_number].addr = vaddr;

		// if page 'p' is not on swap file, assigns a new physical frame 
		// and set dirty bit
		...
		// attempts to load page 'p' from swap file
		} else {
			int pagein_success = swap_pagein(frame_number, p->swap_off);

			if (pagein_success != 0) {
				exit(1);
			}
			
			// shifts frame to make space for status bits; set ON_SWAP bit to 0
			p->frame = frame_number << PAGE_SHIFT;
	...
	// page is valid, so increments hit count
	else {
		hit_count++;
	}
